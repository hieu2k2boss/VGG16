...
def save_data(raw_folder):    
    ...
    dest_size = (128, 128)  # convert image 
    pixels = []
    labels = []
...
#X,y = load_data()
def Chia_Data(X,y,test):
    X_train, X_test, y_train, y_test = train_test_split( X, y, test_size=test, random_state=100)
...
def get_model_VGG16(out):
    ...
    x = Dropout(0.5)(x)
    x = Dense(out, activation='softmax', name='predictions')(x)
...
def Train_Model(out,X_train,y_train,X_test,y_test,epochs):
    vggmodel = get_model_VGG16(out)
